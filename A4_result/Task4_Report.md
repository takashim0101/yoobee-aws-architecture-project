# Task 4: Optimization and Security - Report

## Executive Summary

This report details the implementation of security enhancements, CloudWatch monitoring, and cost optimization strategies for the Yoobee College AWS architecture. Building upon the foundational implementations from previous tasks, this phase focused on hardening the infrastructure, ensuring operational visibility, and identifying avenues for cost reduction. Key outcomes include the establishment of robust network and identity security, comprehensive monitoring for critical events and resource utilization, and a clear strategy for cost optimization, projecting significant annual savings by migrating from an on-premise solution to AWS.

## 1. Security Enhancements Implementation

This section details the practical implementation of infrastructure, network, identity, access management, and data protection security enhancements.
...
*   **Action:** Configure CloudTrail and Amazon EventBridge to set up alerts for critical security events. This is a crucial best practice for maintaining security oversight (Amazon Web Services, 2025a).
*   **Steps:**
...
*   **Steps:**
    ...
    2.  **Interpret Available Checks and Document Findings:**
        *   Accessed the AWS Trusted Advisor console and confirmed the available checks under the Free Tier, specifically:
            *   **MFA on Root Account:** Checks if Multi-Factor Authentication is enabled for the root account.
            *   **Security Groups - Specific Ports Unrestricted:** Checks for security groups with rules that allow unrestricted access (0.0.0.0/0) to specific ports (e.g., SSH:22, RDP:3389).
            *   **Service Limits:** Checks if service usage is approaching 80% of the limit.
...
*   **Action:** Explore AWS Compute Optimizer to receive AI-driven recommendations for rightsizing resources like EC2 instances (Amazon Web Services, 2025b). This ensures you are using the most cost-effective instance types for your workloads.
*   **Steps:**
    ...
    3.  **Interpret Findings and Document Actions (Hypothetical Examples):**
